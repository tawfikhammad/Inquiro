...
async def list_projects(request: Request):
    ...
    if not projects or len(projects) == 0:
        return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=ResponseSignals.PROJECT_NOT_FOUND.value)
    
    serialized_projects = [_serialize_project(project) for project in projects]
    return JSONResponse(status_code=status.HTTP_200_OK, content=serialized_projects)
...
async def get_project(request: Request, project_id: str):
    ...
    return JSONResponse(status_code=status.HTTP_200_OK, content=serialized_project)
...
async def get_project_assets(request: Request, project_id: str):
    ...
    summaries = await summary_model.get_summaries_by_project(project_id=project.id)
